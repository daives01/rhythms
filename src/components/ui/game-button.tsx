    ...
    <div className={cn("flex flex-col items-center gap-2", className)}>
      <span
        className={cn(
          ...
          !warning && active && "text-muted-foreground/50"
        )}
      >
        {label}
      </span>
      ...
      <button
        ...
        onClick={onClick}
        className={cn(
          "group relative w-16 h-16 cursor-pointer overflow-hidden",
      ...
      >
        <div className="absolute inset-1 bg-background shadow-[inset_0_1px_3px_rgba(0,0,0,0.8)] flex items-center justify-center">
          <div
            className="w-10 h-10 transition-all duration-100 bg-muted-foreground group-hover:bg-muted-foreground/80"
            style={{
      ...
      <div
        className={cn(
          ...
          warning && "bg-emerald-950",
          !warning && "bg-emerald-500 shadow-[0_0_8px_rgba(52,211,153,0.7)]",
          !warning && !active && "animate-blink"
        )}
      />
    </div>
  )
